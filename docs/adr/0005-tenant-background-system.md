...
- **Page level**: `LandingPageConfig.background`, sourced from new `landing_pages`
  columns (`background_type`, `background_color_token`, `background_color_custom`,
  ...
  live in. `tenant.ts` assembles the nested object after the query returns, since
  PostgREST can't alias flat columns into nested JSON in a `select()` string.
- **Block level**: an optional `background` attribute (Strapi component
  `shared.background`, identical shape) on `blocks.header`, `blocks.hero`,
  `blocks.differentials`. Absent, or `type: 'transparent'`, means "show whatever's
  behind it" — the page background for an in-flow block, or literally whatever content
  is currently scrolled under a sticky Header.
...
is rendered by a new, reusable `<BackgroundParticles>` component
(`packages/ui/src/components/`) — any block with a background could opt in, not just
...
`resolveBackground(bg)` (`packages/ui/src/utils/background.ts`) maps a `Background` to
a `{ style, classes }` pair for `:style`/`:class` binding — used identically by every
block component and by `useTenantHead.ts` for the page-level `body` rule. Runtime
values (custom hex, resolved gradients, image URLs) can't go through Tailwind's static
utility classes, so background rendering uses inline styles, the same way
`--tenant-primary` already flows through a runtime-injected `<style>` block. `glass` and
`fine-line-texture` use `color-mix(in srgb, <color> X%, transparent)` rather than
hex/rgba math in JS.
...
scrolling content, so `HeaderBlock.vue` falls back to `{ type: 'glass', colorToken:
...
directory (`blocks/hero/HeroDefault.vue`, `HeroCentered.vue`, …) with a two-level
...
- New Supabase migration `20240101000005_add_page_background.sql`; new Strapi component
  `shared.background`; `packages/types` gains `shared.ts` (`Background` and friends).
- `@strapi/plugin-color-picker` (official, version-locked to `@strapi/strapi`) replaces
  raw hex-text inputs for `customColor`/`gradientToCustom`.
...
- Strapi's `landing-page` content-type still carries unused `primaryColor`/
  `secondaryColor`/`fontFamily` fields (the app sources theme from Supabase, not
  Strapi) — the new background fields were **not** added there, following that same
  precedent rather than compounding the drift.
